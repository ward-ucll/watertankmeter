...
DynamicJsonDocument getHardwareInfo() {
  ...
  root["chip_id"] = ESP.getEfuseMac();
  root["flash_chip_size"] = ESP.getFlashChipSize();
  root["cpu_frequency"] = getCpuFrequencyMhz();
...
DynamicJsonDocument getRAMInfo() {
  // Get the total heap size, free heap size, and calculate used heap size
  uint32_t totalHeap = ESP.getHeapSize();
  uint32_t freeHeap = ESP.getFreeHeap();
  uint32_t usedHeap = totalHeap - freeHeap;

  // Create a JSON document and populate it with the RAM information
  const size_t capacity = JSON_OBJECT_SIZE(3); // 3 key-value pairs
  StaticJsonDocument<capacity> doc;
  doc["total_ram"] = totalHeap;
  doc["ram_used"] = usedHeap;
  doc["ram_available"] = freeHeap;
...
DynamicJsonDocument getSdCardInfo() {
  ...
  jsonDoc["cardSize"] = "Card size: " + String(int(SD.cardSize() / (1024.0 * 1024.0))) + "MB";
  jsonDoc["totalSpace"] = "Total space: " + String(int(SD.totalBytes() / (1024.0 * 1024.0))) + "MB";
  jsonDoc["usedSpace"] = "Used space: " + String(int(SD.usedBytes() / (1024.0 * 1024.0))) + "MB";

  // char volumeName[12];
  // if (SD.vol()->getVolumeLabel(volumeName)) {
  //     jsonDoc["volumeName"] = "Volumename: " + String(volumeName);
  // } else {
  //     jsonDoc["volumeName"] = "Error getting volume name.";
  // }
  ...
  if(cardType == CARD_MMC){
    jsonDoc["cardType"] = "MMC" ;
  } else if(cardType == CARD_SD){
    jsonDoc["cardType"] = "SDSC";
  } else if(cardType == CARD_SDHC){
    jsonDoc["cardType"] = "SDHC";
  } else {
    jsonDoc["cardType"] = "UNKNOWN";
  }
...
void setup()
{
    ...
    String jsonString;
    serializeJson(getSdCardInfo(), jsonString);
